...
class MOXMAN_AmazonS3_FileUrlResolver implements MOXMAN_Vfs_IFileUrlResolver {
	...
	public function getFile($url) {
		...
		$bucketname = $this->fileSystem->getBucketOption("bucket");
		$bucketKey = $this->fileSystem->getBucketOption("name");
		$match = MOXMAN_Util_PathUtils::combine($prefix, $bucketname);

		if (strpos($url, $match) === 0) {
			$bucketpath = MOXMAN_Util_PathUtils::combine($prefix, $bucketname);
			$filePath = MOXMAN_Util_PathUtils::combine("s3://" . $bucketKey, substr($url, strlen($bucketpath)));
