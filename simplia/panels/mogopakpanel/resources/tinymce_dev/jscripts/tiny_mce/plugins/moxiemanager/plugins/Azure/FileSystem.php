...
class MOXMAN_Azure_FileSystem extends MOXMAN_Vfs_FileSystem {
	...
	public function __construct($scheme, $config, $root) {
		...
		$containers = $this->getConfig()->get("azure.containers");
		$this->setFileUrlResolver(new MOXMAN_Azure_FileUrlResolver($this));

		// Verify container
		if (!$containers || !isset($containers[$containerName])) {
			throw new MOXMAN_Exception("Could not find container: " . $containerName);
		}

		// Handle development mode
		$container = $containers[$containerName];
		if (isset($container["development"]) && $container["development"]) {
			$container["url"] = 'http://127.0.0.1:10000';
			$container["account"] = 'devstoreaccount1';
			$container["sharedkey"] = 'Eby8vdM02xNOcqFlqUwJPLlmEtlCDXJ1OUzFT50uSRZ6IFsuFq2UVErCz4I6tq/K1SZFPTOtr/KBHBeksoGMGw==';
		}

		// Verify that container has a sharedkey
		if (!isset($container["sharedkey"])) {
			throw new MOXMAN_Exception("No shared key for: " . $containerName);
		}

		$containerName = isset($container["container"]) ? $container["container"] : $containerName;
		if (!isset($container["url"])) {
			$container["url"] = "http://" . $container["account"] . ".blob.core.windows.net";
			$container["urlprefix"] = "http://" . $container["account"] . ".blob.core.windows.net/";
			$container["path"] = "/" . $containerName;
		} else {
			$container["path"] = "/" .  $container["account"] . "/" . $containerName;
			$container["urlprefix"] = "http://localhost:10000/devstoreaccount1/";
		}

		$container["name"] = $url["host"];
		$container["container"] = $containerName;
		$this->container = (object) $container;

		// Setup HTTP client
		$this->httpClient = new MOXMAN_Http_HttpClient($container["url"]);

		// Debug output
		if (isset($container["debug_level"]) && $container["debug_level"] > 0) {
			$this->httpClient->setLogFunction(array($this, "logHttpClient"));
			$this->httpClient->setLogLevel($container["debug_level"]);
		}
	}
	...
	private function signRequest(MOXMAN_Http_HttpClientRequest $request) {
		...
		$request->setHeader("x-ms-date", $current_date);
		...
		$signed .= "/" . $this->container->account . $url["path"];

		if (isset($url["query"])) {
			$queryParts = array();
			$query = parse_str($url["query"], $queryParts);
	 	...
	 	$hash = hash_hmac("sha256", $signed, base64_decode($this->container->sharedkey), true);
		$signature = base64_encode($hash);

		$request->setHeader("Authorization", "SharedKey " . $this->container->account . ":" . $signature);
