...
class MOXMAN_AmazonS3_File extends MOXMAN_Vfs_BaseFile {
	...
	public function copyTo(MOXMAN_Vfs_IFile $dest) {
		...
		if ($dest instanceof MOXMAN_AmazonS3_File) {
			$this->sendXmlRequest(array(
				...
				"amzHeaders" => array(
					"x-amz-copy-source" => '/' . $this->fileSystem->getBucketOption("bucket") . $this->getInternalPath()
				)
			));
		} else {
	...
	public function getUrl() {
		...
		return MOXMAN_Util_PathUtils::combine($prefix, $fileSystem->getBucketOption("bucket") . $this->getInternalPath());
	}
