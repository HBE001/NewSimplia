...
class MOXMAN_AmazonS3_FileSystem extends MOXMAN_Vfs_FileSystem {
	...
	public function __construct($scheme, $config, $root) {
		...
		$bucket["bucket"] = isset($bucket["bucket"]) ? $bucket["bucket"] : $bucketName;
		$this->bucket = $bucket;

		// Setup HTTP client
		$this->httpClient = new MOXMAN_Http_HttpClient($bucket["bucket"] . ".s3.amazonaws.com");
	...
	public function exportTo($path, $localPath) {
		...
		$contentType = "";
		$date = gmdate('r');
		$amzHeaders = "/" . MOXMAN_Util_PathUtils::combine($this->bucket["bucket"], $path);
		$resource = "";
	...
	public function importFrom($localPath, $path) {
		...
		$date = gmdate('r');
		$amzHeaders = "x-amz-acl:public-read\n" . "/" . MOXMAN_Util_PathUtils::combine($this->bucket["bucket"], $path);
		$resource = "";
	...
	public function sendRequest($params) {
		...
		$amzHeaders .= "/" . $this->bucket["bucket"] . $params["path"];
