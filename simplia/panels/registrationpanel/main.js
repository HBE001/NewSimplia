...
RegistrationPanel.prototype.getTableData = function(callback) {
    ...
    tableData.push(this.addTableDataRow({type: 'text', value: "Email"}, {type: "textbox", value:''}));
    tableData.push(this.addTableDataRow({type: 'text', value: "Password"}, {type: "password", value:''}));
    tableData.push(this.addTableDataRow({type: 'text', value: "Confirm Password"}, {type: "password", value:''}));
    tableData.push(this.addTableDataRow({type: 'text', value: "Register"}, {type: "button", name: "Register", callback: function(){ that._register();}}));
...
RegistrationPanel.prototype._register = function(){
    ...
    async.waterfall([
        function(callback) {
            ...
            var registrationData = {command: "registerAccount", role: that.role, data: that.collectFormData()};
            that.doAjaxJSONCall("./" + that.typeNodeId + "/" + that.serverNodeId, {command: "relayServerTip", data: JSON.stringify(registrationData)}, function(data){
                callback(null, data);
            });
        }
    ], function(error, data){
        ...
        if(data.error) {
            var error = ( $.isPlainObject(data.errorInfo) ) ? (JSON.stringify(data.errorInfo)) : data.errorInfo;
            that._setStatus(error);
        }
        else {
            that.setFormValue("Username", {type: "textbox", value: data.accountItem.DisplayName });
            that._setStatus(data.status);
        }
    });
};
