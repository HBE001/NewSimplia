...
function BasicFormDatatablePanel(Y, properties) {
    ...
    this._valueColIndex = 1;
    this.OESEditorPanelName = "oeseditor";
    this.OESEditorPanelType = "oesinputformpanel";

    this.GDNGeneratorPanelName = "generategdnform";
    this.GDNGeneratorPanelType = "generategdnformpanel";
...
BasicFormDatatablePanel.prototype.init = function(cb) {
    var that = this;
    if(typeof this.rawFormData !== "undefined" && $.isArray(this.rawFormData)) {
        this.getTableData = function(callback) {
            //console.log('rawFormData:', that.rawFormData);
            //console.log('length:', that.rawFormData.length);
            var tableData = [];
            that.rawFormData.forEach(function(rawRow){
                tableData.push(that.addTableDataRow.apply(that, rawRow));
            });
...
BasicFormDatatablePanel.prototype._formFormatter = function(o) {
    ...
    var formatting = {
        ...
        'textbox-id': function(o) {
            ...
            that._buttonHandlers[uid] = that._getGUID.bind(that, function(guid){ $('#txt_' + uid).val(guid)});
        ...
        'textbox-ost': function(o) {
            ...
            var html = '<input type="text" id="txt_' + uid + '" value="' + o.value.value + '"/>';
            that._buttonHandlers[uid] = that._generateOST.bind(that, function(ost){ $('#txt_' + uid).val(ost)});
        ...
        'textbox-oes': function(o) {
            ...
            var html = '<input type="text" id="txt_' + uid + '" value="' + o.value.value + '"/>';
            that._buttonHandlers[uid] = that._displayOESEditor.bind(that, function(oes){ $('#txt_' + uid).val(oes)});
        ...
        'textbox-odn': function(o) {
            ...
            var html = '<input type="text" id="txt_' + uid + '" value="' + o.value.value + '"/>';
            that._buttonHandlers[uid] = that._displayGDNGenerator.bind(that, function(gdn){ $('#txt_' + uid).val(gdn)});
...
BasicFormDatatablePanel.prototype.getFormFieldValue = function(key, skip) {
    var formData = this.collectFormData(skip || undefined);
...
BasicFormDatatablePanel.prototype._displayGDNGenerator = function(callback) {
    var childPanel = this.childPanel(this.GDNGeneratorPanelName);
    //console.log('childPanel:', childPanel);
    ...
    if(childPanel) {
        ...
        this.addChildPanel(this.GDNGeneratorPanelType, this.GDNGeneratorPanelName, properties, function(error, panel){
            if(error) {
