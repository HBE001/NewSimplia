...
$(".FormContent").append("<div class='editor_holder'></div>" +
    "<button class='saveTransition'>Save Transition</button>" +
    "<br/> <br/> " +
    "<div class= 'machine'> " +
    "<div class='machine_holder'></div>" +
    "<button class='run'>Run</button> " +
    "</div> " +
    "<br/> <br/>" +
    "<div id= 'content' class='jsonContent'></div>");
...
function setupBFSTMachine(currentState, currentRole) {
    ...
    loadScript("https://rawgit.com/jdorn/json-editor/master/dist/jsoneditor.js", function () {
        if (roleStateJSON != undefined && roleStateJSON['RoleState'] != undefined
            && roleStateJSON['RoleState'][currentRole] !== undefined
...
loadScript("https://rawgit.com/jdorn/json-editor/master/dist/jsoneditor.js", function () {
    ...
    $('.saveTransition').on('click', function () {
        ...
        var loadedContextData = JSON.parse(that.loadedcontext.data);
        var stateInputJSON = loadedContextData['stateInputJSON'];
        var roleStateJSON = loadedContextData['roleStateJSON'];

        var jsonValue = that.collectFormData(['Save Button']);
        ...
        if (stateInputJSON == undefined || stateInputJSON['StateInput'] == undefined) {
        ...
        loadedContextData['stateInputJSON'] = stateInputJSON;
        loadedContextData['roleStateJSON'] = roleStateJSON;

        that.loadedcontext.data = JSON.stringify(loadedContextData);

        that.loadedDataItem[that.context_parameter] = that.loadedcontext;

        var itemData = {};
        var dataToSave = {
            tableName: that.tableName,
            data: JSON.stringify(that.loadedDataItem)
        };
        console.log('Data TO Save = ');
        console.log(dataToSave);
        that.doAjaxJSONCall(that.saveUrl, $.param(dataToSave));
