...
Baselib.prototype.getNodeId = function () {
    return this.config.get('config.nodeId.sectorId') + this.config.get('config.nodeId.separator') + this.config.get('config.nodeId.serviceId') + this.config.get('config.nodeId.separator') + this.getCurrentTime() + this.config.get('config.nodeId.separator') + this.config.get('config.nodeId.count');
};
...
Baselib.prototype.addAnonymousUser = function (docClient, item, cb) {
    var putItemParams = {
        Item: item,
        TableName: this.config.get('config.dynamodb.tables.anonymoususers.tableName')
...
Baselib.prototype.createAnonymousUserItem = function (req) {
    ...
    var nodeId = item[this.config.get('config.dynamodb.tables.anonymoususers.hashKey')] = this.getNodeId();
    item[this.config.get('config.dynamodb.tables.anonymoususers.rangeKey')] = this.config.get('config.dynamodb.fixedValues.attributesEdge');
        ...
        function(data, callback){
            ...
            if(data.Count > 0) {
                var updateItemParams = {
                    ...
                    TableName: that.config.get('config.dynamodb.tables.anonymoususers.tableName')
                };
                updateItemParams.Key[that.config.get('config.dynamodb.tables.anonymoususers.hashKey')] = data.Items[0][that.config.get('config.dynamodb.tables.anonymoususers.hashKey')];
                updateItemParams.Key[that.config.get('config.dynamodb.tables.anonymoususers.rangeKey')] = that.config.get('config.dynamodb.fixedValues.attributesEdge');
...
Baselib.prototype.getAnonymousUser = function (docClient, userId, cb) {
    var params = {
        TableName: this.config.get('config.dynamodb.tables.anonymoususers.tableName'),
        IndexName: this.config.get('config.dynamodb.tables.anonymoususers.indexes.accountName'),
...
Baselib.prototype.setInitialCookies = function (req, res, cookiesList) {
    ...
    for (var i in cookiesList) {
        res.cookie(i, cookiesList[i], {path: this.config.get('config.cookies.path'), domain: this.config.get('config.cookies.domain')});
    }
    res.cookie('referer', referer, {path: this.config.get('config.cookies.path'), domain: this.config.get('config.cookies.domain')});
};
...
Baselib.prototype.getDynamicTungstenExt = function (accountId, cb) {
    //request(config.get('config.tungstenExtensionAPI.allocateDynamicExt'), function(error, response, body){
    console.log('request', JSON.parse(mustache.render(JSON.stringify(config.get('config.tungstenExtensionAPI.allocateDynamicExt')), {
        account_id: accountId,
        server_identity: config.get('config.nodeId.sectorId')
    })));
    request(JSON.parse(mustache.render(JSON.stringify(config.get('config.tungstenExtensionAPI.allocateDynamicExt')), {
        account_id: accountId,
        server_identity: config.get('config.nodeId.sectorId')
...
Baselib.prototype.getTungstenExtension = function (accountId, cb) {
    request(JSON.parse(mustache.render(JSON.stringify(config.get('config.tungstenExtensionAPI.provisionAccountExt')), {account_id: accountId})), function (error, response, body) {
