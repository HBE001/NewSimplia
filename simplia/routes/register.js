...
module.exports = function(app, config) {
    ...
    app.post('/register', function (req, res) {
        async.waterfall([
            ...
            function(data, callback) {
                updateAccount(data.accountData.Item[config.get('config.dynamodb.tables.accounts.hashKey')], createAccountPutUpdate({ZimbraEmail: data.mailData.email}), callback);
            }
        ], function(err, data){
            if (err) {
    ...
    app.post('/registerdeveloper', function (req, res) {
        ...
        async.waterfall([
                ...
                function(zimbraData, callback) {
                    ...
                    request.post({
                        url: config.get('config.developersetup.url'),
                        form: {
                            email: email,
                            password: password,
                ...
                function(response, body, callback) {
                    ...
                    request.post({
                        url: config.get('config.cognitoadd.url'),
                ...
                function(response, body, callback){
                    ...
                    request.get({
                        url: config.get('config.portassignservice.url') + config.get('config.portassignservice.developmentServerName') +
                            "/" + config.get('config.portassignservice.portTypes.application') + "/" +
                            config.get('config.portassignservice.defaultNumPorts') + "/" + email
                    }, callback);
                },
                ...
                function(response, body, callback){
                    ...
                    request.get({
                        url: config.get('config.portassignservice.url') + config.get('config.portassignservice.developmentServerName') +
                            "/" +config.get('config.portassignservice.portTypes.debug') + "/" + config.get('config.portassignservice.defaultNumPorts')
                            + "/" + email
                    }, callback);
                },
                ...
                function(response, body, callback){
                    ...
                    updateAccount(
                        dbAccountData.Item[config.get('config.dynamodb.tables.accounts.hashKey')],
    ...
    var createAccountItem = function(req, customItems) {
        var item = {
            ...
            AccountPassword: bcrypt.hashSync(req.body.password || req.body.Password, config.get('config.passwordEncryption.saltLength')),
            UserAttributes: {
    ...
    var updateAccount = function(nodeId, update, cb) {
        ...
        updateItemParams.Key[config.get('config.dynamodb.tables.accounts.hashKey')] = nodeId;
