...
module.exports = function(app, config) {
    var capability = new twilio.Capability(config.get('twilio.accountSid'), config.get('twilio.authToken'));
    var lastUser = "";

    app.post('/twilio/gettoken', function(req, res){
        console.log(req.body);
        capability.allowClientIncoming(req.body.username);
        capability.allowClientOutgoing(config.get('twilio.simpliaLine.SID'));
    ...
    app.post('/twilio/outgoingcall', function(req, res){
        ...
        if(typeof req.body['PhoneNumber'] !== "undefined") {
            resp.dial({callerId: config.get('twilio.simpliaLine.number')}, function (node) {
